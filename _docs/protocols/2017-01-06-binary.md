...
but the size of that list is guaranteed to be at list `1`.
...
`MessageName` is encoded in same manner as `ByteString`. Note that encoded `ByteString` is itself encoded `UnsignedVarInt`, or concatenation of two encoded
`UnsignedVarInt`s.
