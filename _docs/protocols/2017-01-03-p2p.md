...
network. Later, the node will attempt to find more peers by querying its neighbors
(from the initial list of peers sent by the bootstap node). A node sends messages
to its peers, which resend messages to their peers close to the needed ID/key.
...
any problems for old nodes (with a possible exception of some network overhead),
