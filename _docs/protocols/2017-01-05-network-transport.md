...
Please read [Cardano SL Implementation
Overview](/for-contributors/implementation) for more info. This guide covers the
network transport layer used in Cardano SL nodes.
...
-   **Single TCP connection**. Only a single TCP connection at a time is used
    ...
    or some unrecoverable error occurred.
...
endpoints are multiplexed on a single heavyweight connection (ie a single TCP
connection).
...
local state that it now has an established (i.e. no longer initialising)
...
A `ConnectionRequestCrossed` response occurs when endpoint B determined that an
existing TCP connection already exists between A and B, or connections between A
and B and B and A were being established concurrently. In this case both
endpoints must close the TCP connection.
...
crossed connection situation. The second case can also occur legitimately
(i.e. not a protocol violation) when one peer has discovered that the existing
TCP connection has failed (i.e. its end is closed) and is trying to establish a
new TCP connection, while the other peer has not yet discovered that the
existing TCP connection is dead.
