...
Here and later, under **address** we mean tuple `(Host, Port, ID)`, while
**network address** would denote to just pair `(Host, Port)`.
...
As ID we use the so-called `HashNodeId`. Since it [contains](#id-structure)
