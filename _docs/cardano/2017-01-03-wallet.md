...
| `send`        | `from, to :: CAddress; amount :: Coin` | `Post '[JSON] (Either WalletError CTx) | Send coins in the default currency (presently, `ADA`) from an origin address to a destination address, without any transaction message or description.[1] |
 `send`        | `from, to :: CAddress; amount :: Coin; currency :: CCurrency; title :: Text; description :: Text` | `Post '[JSON] (Either WalletError CTx) | Send an amount of coins in a specified currency from an origin address to a destination address.[1] |
| `txhistory` | `address :: CAddress; skip :: Word; limit :: Word` | `Get '[JSON] (Either WalletError ([CTx], Word))` | Fetch a tuple with the list of transactions where the address took part in the index interval [skip + 1, limit], and its length.[2] |
| `search-txhistory` | `address :: CAddress; search :: Text; skip :: Word; limit :: Word | `Get '[JSON] (Either WalletError ([CTx], Word))` | Fetch a tuple with the list of transactions whose title has `search` as an infix, in the index interval [skip + 1, limit], and its length.[2]|
| `update-transaction` | `address :: CAddress; transaction :: CTxId; ReqBody '[JSON] CTxMeta` | `Post '[JSON] (Either WalletError ())` | Add the transaction which has the given ID to the wallet's transaction history, if such a transaction exists. |
...
| `valid-address` | `currency :: CCurrency; address :: Text` | `Get '[JSON] (Either WalletError Bool)` | Reply with `True` if the address is valid, and `False` otherwise.[4] |
...
[5]: `CProfile` has several fields, among them: the user's name, their email,
their phone number, the hash of their password, the `POSIX`-formatted time at
which the account was created, their location, and the account's picture.
[6]: An `ADA` redemption token is represented by the datatype `CWalletRedeem`,
which has the address where the `ADA` intended to be redeemed will go to, and a
redemption seed.
