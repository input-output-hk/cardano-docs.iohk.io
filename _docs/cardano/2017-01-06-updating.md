...
* A **soft fork** is a consensus fork wherein everything that was previously
  invalid remains invalid while blocks that would have been previously considered
  valid become invalid.
* A **hard fork** is a consensus fork that makes previously invalid blocks valid.
...
Another problem is that in our implementations all blocks have a field called
...
would be polluted with meaningless keys. We would have to accept it, and that
would bloat the blockchain.
...
* **Confirmed**, when there is an update proposal that contains a confirmed
  ...
  `2.0.0`, `2.0.1`, `1.2.0` and `1.2.1`. `1.1.1` and `1.1.2` aren't competing,
  because `1.1.3` is already adopted.
...
1. Once the update is approved, the protocol version (say `0.5.0`) can be used.
2. Before a soft fork is resolved (i.e. the resolution rule hit), the updated
   nodes should issue blocks with the new version `0.5.0`, but treat and validate
   blocks of version `0.5.0` as blocks of version `0.4.0`.
3. Non-updated updated nodes accept new blocks:
    1. Before the soft fork is resolved, discarding as per `0.4.0`.
    2. After the soft fork is resolved, as per `0.5.0` (i.e. allowing a block to
