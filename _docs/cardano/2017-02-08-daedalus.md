...
| `sendExtended` | `from, to :: String;`<br/> `amount :: Int;`<br/> `title :: String;`<br/> `description :: String;` | `Promise CTx` | Send coins in the default currency (presently, `ADA`) from an origin address to a destination address, without any transaction message or description. `[1]`
...
| `updateTransaction` | `address :: String;`<br/> `transactionId :: String;`<br/> `currency :: String;`<br/> `title :: String;`<br/> `description :: String;`<br/> `date :: Number;` | `Promise Unit` | Add the transaction which has the given ID to the wallet's transaction history, if such a transaction exists.
| `generateMnemonic` | Nothing | `String` | Generate a BIP39 mnemonic for the wallet. [4]
| `isValidMnemonic` | `mnemonic :: String` | `Boolean` | Verify that the given mnemonic is valid as per BIP39 rules.
...
| `restoreWallet` | `walletType :: String;`<br/> `currency :: String;`<br/> `walletName :: String;` | `Promise CWallet` | Recover the wallet associated to the given backup information `[3]`, if it exists.
| `restoreWalletIgnoreChecksum` | `walletType :: String;`<br/> `currency :: String;`<br/> `walletName :: String;` | `Promise CWallet` | Recover the wallet associated to the given backup information `[3]`, if it exists. Ignore whether the mnenonic is valid or not.[4]
...
| `getProfile` | Nothing | `Promise CProfile` | Fetch the client's current user profile - the datatype `CProfile`. `[6]`
...
| `redeemAda` | `address, seed :: String` | `Promise CWallet` | Redeem ADA from a token `[7]`, create and return a wallet with the redeemded ADA.
...
| `notify` | `forall eff. message :: (NotifyCb eff);`<br/> `error :: (ErrorCb eff);` | Unit | Notify the user of an event.
...
| `postponeUpdatesUntil` | `time :: Number` | `Promise Unit` | Postpone any updates to the system until the given date.
|---
...
* `[4] : https://github.com/bitcoinjs/bip39#reminder-for-developers
* `[5]`: Presently, any currency other than `ADA` results in a `False`.
* `[6]`: `CProfile` has several fields, among them: the user's name, their email, their phone number, the hash of their password, the `POSIX`-formatted time at which the account was created, their location, and the account's picture.
* `[7]`: An `ADA` redemption token is represented by the datatype `CWalletRedeem`, which has the address where the `ADA` intended to be redeemed will go to, and a redemption seed.
